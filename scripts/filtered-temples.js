...
function createTempleCard(filteredTemples) {
    ...
    filteredTemples.forEach(temple => {
        ...
        title.innerHTML = `<span class ="label">Location:</span>`;
...
homelink.addEventListener("click", () => {createTempleCard(temples)});

const oldLink = document.querySelector("#old");
oldLink.addEventListener("click", () => {
    const cutDate = new Date("1900, January, 01");
    const oldTemples = temples.filter(temple => {
        const dedicationDate = new Date(temple.dedicated);
        return dedicationDate < cutDate;
    })
...
newLink.addEventListener("click", () => {
    const cutDate = new Date("1999, December, 31");
